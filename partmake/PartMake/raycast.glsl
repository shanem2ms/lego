...
void main()
{
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        vec2 p = (2.0*(fragCoord+o)-res.xy)/res.y;
#else    
        vec2 p = (2.0*fragCoord-res.xy)/res.y;
#endif
        ...
        vec3 rd = ca * normalize( vec3(p,fl) );
         ...
         // ray differentials
        vec2 px = (2.0*(fragCoord+vec2(1.0,0.0))-res.xy)/res.y;
        vec2 py = (2.0*(fragCoord+vec2(0.0,1.0))-res.xy)/res.y;
